...
pub fn linkBearSSL(path_prefix: []const u8, module: *std.Build.Step.Compile, target: std.Build.ResolvedTarget, b: *Builder) void {
    ...
    defer {
        for (paths) |p| {
            module.step.owner.allocator.free(p);
        }
...
fn addPrefixToPaths(b: *std.Build, prefix: []const u8, paths: []const []const u8) []const []const u8 {
    var prefixed_paths = std.ArrayList([]const u8).init(b.allocator);
    for (paths) |p| {
        prefixed_paths.append(b.pathJoin(&[_][]const u8{ prefix, p })) catch @panic("OOM");
    }
    const new_paths = prefixed_paths.toOwnedSlice() catch @panic("OOM");
    return new_paths;
}
...
pub const DERCertificate = struct {
    ...
    fn fromX509(allocator: std.mem.Allocator, cert: *c.br_x509_certificate) !DERCertificate {
        return Self{
            .allocator = allocator,
